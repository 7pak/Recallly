...
@Composable
fun SettingsWorkScheduleScreen(
    uiState: SettingsUiState,
    onEvent: (SettingsUiEvent) -> Unit,
    onNavigateBack: () -> Unit
) {
    LaunchedEffect(uiState.scheduleSaved) {
        if (uiState.scheduleSaved) {
            onEvent(SettingsUiEvent.ResetScheduleSaved)
            onNavigateBack()
        }
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Text(
                        text = stringResource(R.string.settings_work_schedule_title),
        ...
        Column(
            ...
            DaySelector(
                ...
                onToggleDay = { onEvent(SettingsUiEvent.ToggleWorkDay(it)) }
            )
            ...
            Row(
                ...
                TimeSelector(
                    ...
                    onTimeSelected = { onEvent(SettingsUiEvent.SetStartTime(it)) },
                    modifier = Modifier.weight(1f)
                )
                TimeSelector(
                    label = stringResource(R.string.schedule_end),
                    time = uiState.endTime,
                    onTimeSelected = { onEvent(SettingsUiEvent.SetEndTime(it)) },
                    modifier = Modifier.weight(1f)
                )
            }
            ...
            Button(
                onClick = { onEvent(SettingsUiEvent.SaveSchedule) },
                modifier = Modifier
                    .fillMaxWidth()
...
@Composable
private fun buildScheduleSummary(state: SettingsUiState): String {
